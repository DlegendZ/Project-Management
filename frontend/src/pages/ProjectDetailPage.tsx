...
export function ProjectDetailPage() {
  ...
  const handleCreateTask = async (
    data: import("../types").CreateTaskRequest,
  ) => {
      ...
      <header className="flex-shrink-0 flex items-center gap-4 px-6 py-4 border-b border-slate-700 bg-slate-900">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <h1 className="text-slate-100 font-semibold text-lg truncate">
              {project.name}
            </h1>
        ...
        <Select
          ...
          onChange={(e) =>
            setFilters((f) => ({
              ...f,
              priority:
                (e.target.value as TaskFilters["priority"]) || undefined,
            }))
          }
        />
        <Select
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Project Settings">
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <div className="flex justify-end gap-3">
          ...
          <Button type="submit" loading={loading} icon={<Pencil size={14} />}>
            Save
          </Button>
        </div>
      </form>
...
function MembersModal({
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(
        axiosErr.response?.data?.error?.message ?? "Failed to add member",
      );
    } finally {
      setAddLoading(false);
    }
