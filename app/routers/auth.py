...
def register(data: RegisterRequest, db: Session = Depends(get_db)):
    user = UserService.create_user(db, username=data.username, email=data.email, password=data.password)
    return user
...
def login(data: LoginRequest, db: Session = Depends(get_db)):
    ...
    access_token = AuthService.create_access_token(user.id, user.role.value, user.token_version)
...
def logout(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    UserRepository.update(db, current_user, token_version=current_user.token_version + 1)
