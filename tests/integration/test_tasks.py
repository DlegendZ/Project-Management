...
def test_create_task_as_non_member_returns_403_or_404(
...
def test_get_task_success(
    ...
    resp = client.get(
        f"/api/v1/projects/{test_project.id}/tasks/{test_task.id}", headers=user_headers
    )
...
def test_list_tasks_with_pagination(
    ...
    resp = client.get(
        f"/api/v1/projects/{test_project.id}/tasks?limit=1&offset=0",
        headers=user_headers,
    )
    assert resp.status_code == 200
    data = resp.json()
...
def test_list_tasks_filtered_by_status_returns_correct_items(
    ...
    resp = client.get(
        f"/api/v1/projects/{test_project.id}/tasks?status=todo", headers=user_headers
    )
    assert resp.status_code == 200
...
def test_list_tasks_filtered_by_priority(
    ...
    resp = client.get(
        f"/api/v1/projects/{test_project.id}/tasks?priority=medium",
        headers=user_headers,
    )
    assert resp.status_code == 200
    for item in resp.json()["items"]:
...
def test_list_tasks_search_by_keyword(
    ...
    resp = client.get(
        f"/api/v1/projects/{test_project.id}/tasks?q=SearchableTask_XYZ",
        headers=user_headers,
    )
    assert resp.status_code == 200
    data = resp.json()
...
def test_list_tasks_sort_by_due_date(
    client: TestClient, user_headers: dict, test_project
):
    resp = client.get(
        f"/api/v1/projects/{test_project.id}/tasks?sort_by=due_date&sort_dir=asc",
        headers=user_headers,
    )
    assert resp.status_code == 200
...
def test_delete_task_as_creator(
    client: TestClient, user_headers: dict, test_project, db: Session, test_user
):
    ...
    db.commit()
    resp = client.delete(
        f"/api/v1/projects/{test_project.id}/tasks/{t.id}", headers=user_headers
    )
...
def test_create_task_past_due_date_returns_400(
    client: TestClient, user_headers: dict, test_project
):
